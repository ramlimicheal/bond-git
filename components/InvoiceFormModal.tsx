...
interface InvoiceFormModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (invoice: Omit<Invoice, 'id'>, invoiceId?: string) => void;
    editInvoice?: Invoice | null; // If provided, modal is in edit mode
}
...
export const InvoiceFormModal: React.FC<InvoiceFormModalProps> = ({
    ...
    const [status, setStatus] = useState<'Pending' | 'Paid' | 'Draft' | 'Overdue'>('Pending');
    ...
    useEffect(() => {
        if (isOpen && editInvoice) {
            ...
            setClientType(editInvoice.clientType);
            setStatus(editInvoice.status as any);
            setInvoiceNumber(editInvoice.number);

            // Convert display dates back to input format (YYYY-MM-DD)
            const parseDueDate = new Date(editInvoice.dueDate);
            const parseIssuedDate = new Date(editInvoice.issuedDate);
            setDueDate(parseDueDate.toISOString().split('T')[0]);
            setIssuedDate(parseIssuedDate.toISOString().split('T')[0]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const invoiceData = {
            number: invoiceNumber || `INV-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`,
            status: status,
            clientName,
            clientType,
            issuedDate: formatDate(issuedDate || new Date().toISOString()),
        ...
        onSubmit(invoiceData, editInvoice?.id);
        onClose();
    };
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            {isEditMode ? `Editing ${editInvoice?.number}` : 'Create a new invoice for your customer'}
                        </p>
                    </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                                        {['Pending', 'Paid', 'Draft', 'Overdue'].map((s) => (
                                            <button
                                                key={s}
                                                type="button"
